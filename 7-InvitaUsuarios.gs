...
var response = ui.alert('Quieres continuar con SOLO AGREGAR INVITADOS?', 
ui.ButtonSet.YES_NO);
// Process the user's response.
 if (response == ui.Button.YES) {
   Logger.log('The user clicked "Yes."');
      ...
      data = data.filter(function (r) {return r[16] == "Invitar" && r[17] != "Invitado"});
      Logger.log(data);
    ...
    for (var i = 0;i<data.length;i++){
      ...
      try {
        ...
        cal.getEventSeriesById(idEvento)
          ...
          .addGuest(invitadoUsuario)
          Logger.log(idEvento+' :Completado');

        ss.getRange(filaEscribir, columnaEscribir).setValue("Invitado");
